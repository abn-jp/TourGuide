<?php
    ...
    if (json_last_error() != JSON_ERROR_NONE || !isset($data['itinerary']) || !is_array($data['itinerary'])) {
        $response['error'] = true;
    ...
    try {
        ...
        $itineraryInsertResult = $tour->insertTourItinerary($data['itinerary']);

        if ($itineraryInsertResult <= 0) {
            throw new \Exception("48: Tour itinerary insertion failed!", 1);
        }

        if ($dbcon->commit()) {
            $response['error'] = false;
            $response['message'] = "Tour itinerary added successfully!";
        } else {
