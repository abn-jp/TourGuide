<?php
    ...
    if (json_last_error() != JSON_ERROR_NONE || !isset($data['included']) || !is_array($data['included'])) {
        $response['error'] = true;
    ...
    try {
        ...
        $insertResult = $tour->insertTourIncluded($data['included']);

        if ($insertResult <= 0) {
            throw new \Exception("48: Tour included insertion failed!", 1);
        }

        if ($dbcon->commit()) {
            $response['error'] = false;
            $response['message'] = "Tour included added successfully!";
        } else {
