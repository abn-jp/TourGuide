<?php
    ...
    if (json_last_error() != JSON_ERROR_NONE || !isset($data['highlights']) || !is_array($data['highlights'])) {
        $response['error'] = true;
    ...
    try {
        ...
        $insertResult = $tour->insertTourHighlights($data['highlights']);

        if ($insertResult <= 0) {
            throw new \Exception("48: Tour highlights insertion failed!", 1);
        }

        if ($dbcon->commit()) {
            $response['error'] = false;
            $response['message'] = "Tour highlights added successfully!";
        } else {
